...
int main() 
{
	...
	Size board_size = Size(9,6);    /* 标定板上每行、列的角点数 ,方格内部的点（比如方格是7行10列，那么就是6*9的size）*/
	...
	while (getline(fin,filename))
	{
		...
		/* 输出检验*/
		// CString c_filename = filename.c_str();
		// filename = to_string(c_filename.Replace("\n",""));
		// Doreplace(filename,"\n","");
		// Doreplace(filename,"\r","");
		// Doreplace(filename,"\t","");
		cout<<"-->count = "<<count <<":::"<<  trim(filename);		
		// cout<<"--= "<<endl;		

		Mat imageInput=imread(trim(filename));
	...
	*************************************************************************/
	...
	for (int i = 0 ; i != image_count ; i++)
	{
		...
		filePath+=".JPG";
		cout << filePath << "---" << endl;
		Mat imageSource = imread("/home/disk2/workspace/Scheaven/01_CPP_project/09_Merge_img/03_cam_calibration/camera_calibration_tool/chess/cT_20200720153900.JPG");
